...
def juce_poll_status(session_id):
    """Poll for session status, progress, and results - now with real-time queue status"""
    try:
        ...
        # NEW: Get queue status - prefer stored updates from /task_notification over direct Go service query
        queue_status = get_stored_queue_status(session_id)
        
        # Fallback to direct Go service query if no stored status found
        if not queue_status:
            try:
                response = requests.get(f'http://gpu-queue:8085/tasks/{session_id}')
                if response.status_code == 200:
                    go_service_data = response.json()
                    # Convert Go service response to our queue status format
                    if go_service_data:
                        queue_status = get_queue_status_message({'queue_status': go_service_data})
                        queue_status['session_id'] = session_id
                        queue_status['source'] = 'go_service_direct'
                        print(f"[HTTP] Using direct Go service queue status for {session_id}")
            except Exception as e:
                print(f"[HTTP] Error querying Go service for {session_id}: {e}")
                queue_status = {}
        else:
            queue_status['source'] = 'stored_update'
            print(f"[HTTP] Using stored queue status for {session_id}")
        
        # Get completed audio if processing is done
        audio_data = None
        if not generation_in_progress and not transform_in_progress:
            audio_data = retrieve_audio_data(session_id, 'last_processed_audio')
        ...
        return jsonify({
            ...
            'audio_data': audio_data,
            'queue_status': queue_status,  # Now includes real-time updates from /task_notification
            'session_data': {
                'model_name': session_data.get('model_name'),
                'prompt_duration': session_data.get('prompt_duration'),
                'parameters': session_data.get('parameters', {}),
