...
class RotaryEmbedding(nn.Module):
    ...
    def rotate(self, x: torch.Tensor, start: int = 0, time_dim: int = 1, invert_decay: bool = False):
        ...
        if x.device.type == 'mps':
            scaled_cos = (rotation.real * decay) * self.scale + (1.0 - self.scale)
            scaled_sin = (rotation.imag * decay) * self.scale + (1.0 - self.scale)
            scaled_cos = repeat(scaled_cos, '... n -> ... (n r)', r = 2)
            scaled_sin = repeat(scaled_sin, '... n -> ... (n r)', r = 2)
            x_out = (x.to(self.dtype) * scaled_cos) + (self.rotate_half(x.to(self.dtype)) * scaled_sin)
        else:
