...
from model_cache import cached_model, get_model_cache_info, clear_model_cache
...
def _process_audio_impl_v2(
    ...
    try:
        ...
        with enforced_dtype_context():
            # Keep full device string for tensor operations
            device = f"cuda:{device_id}" if torch.cuda.is_available() else "cpu"
            
            # But use simple "cuda" for model cache (autocast compatibility)
            ...
            # Use cache_device (just "cuda") instead of device ("cuda:0")
            with cached_model(model_name, cache_device) as model:
...
def _continue_music_impl_v2(
    ...
    """
    Music continuation with model caching.
    Model stays in cache (CPU) instead of being deleted.
    """
    try:
        song, sr = load_and_validate_audio(input_data_base64)
        ...
        with enforced_dtype_context():
            ...
            # Use simple "cuda" for model cache (autocast compatibility)
            cache_device = "cuda" if torch.cuda.is_available() else "cpu"
            ...
            # Use cached_model context manager
            with cached_model(model_name, cache_device) as model:
                if progress_callback:
                    model.set_custom_progress_callback(progress_callback)
        ...
        # No model deletion needed - it's in cache!
        if torch.cuda.is_available():
            torch.cuda.empty_cache()
        gc.collect()
